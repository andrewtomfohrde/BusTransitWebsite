...
public class SmallBusColorDecoratorTest {
  ...
  public void testGetColorDoesNotDelegate() {
    ...
    SmallBusColorDecorator deco = new SmallBusColorDecorator(wrapped, 180);
    deco.getColor();
  ...
  public void testUpdateDelegatesToWrappedAndCopiesState() {
    ...
    when(wrapped.getName()).thenReturn("MiniBus");

    SmallBusColorDecorator deco = Mockito.spy(new SmallBusColorDecorator(wrapped, 100));

    deco.update();
    verify(wrapped).update();

    assertEquals(pos, deco.getPosition());
    assertEquals("MiniBus", deco.getName());
  }
  ...
  public void testDelegatedMethods() {
    ...
    SmallBusColorDecorator deco = new SmallBusColorDecorator(wrapped, 255);
